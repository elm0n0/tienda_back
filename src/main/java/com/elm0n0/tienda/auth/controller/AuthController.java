package com.elm0n0.tienda.auth.controller;
...
import com.elm0n0.tienda.auth.dto.request.LogOutRequest;
...
import com.elm0n0.tienda.auth.service.AuthService;
...
public class AuthController {
	...
	*/
	...
	public ResponseEntity<Object> registrarUsuario(
		@RequestBody RegisterRequest usuarioRequest,
		@RequestHeader("User-Agent") String deviceName
			) {
		try {
			AuthResponse token = usuarioService.registrarUsuario(usuarioRequest, deviceName);
	...
	public ResponseEntity<Object> login(
		...
		@RequestHeader("User-Agent") String deviceName) {
		try {
			AuthResponse token = usuarioService.login(loginRequest, deviceName);
			return ResponseEntity.ok(token);
		} catch (AuthException e) {
			return ResponseEntity.status(e.getMensaje().codigo()).body(e.getMensaje());
		}
	...
	public ResponseEntity<Object> refreshToken(
		...
		try {
			return ResponseEntity.ok(usuarioService.refreshToken(refreshToken, deviceName));
		} catch (AuthException e) {
			return ResponseEntity.status(e.getMensaje().codigo()).body(e.getMensaje());
		}
	}
	...
	public ResponseEntity<Object> logout(
			...
			@RequestHeader("User-Agent") String deviceName) {
		try {
			return ResponseEntity.ok(usuarioService.logout(logOutRequest.email() ,deviceName));
		} catch (AuthException e) {
			return ResponseEntity.status(e.getMensaje().codigo()).body(e.getMensaje());
		}
	}
